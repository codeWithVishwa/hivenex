...
The frontend is a **static SPA on Vercel** and the API is a **web service on
Render**; they live on different origins. Configs: [`vercel.json`](vercel.json)
(static + SPA rewrite) and [`render.yaml`](render.yaml) (backend Blueprint).
...
**Frontend on Vercel** — Add New → Project → import the repo (auto-detects
Vite; `vercel.json` serves the SPA). Add **one env var** so the app calls
Render instead of its own origin:

```
VITE_API_URL=https://<your-service>.onrender.com/api
```

Redeploy after setting it (Vite bakes env vars in at build time). Then set
**`CLIENT_ORIGIN`** on the Render service to your Vercel URL (comma-separate
multiples). Leaving it unset allows any origin — safe here since the API
authenticates with JWTs (not cookies), but setting it is tidier.

> On the **free** Render plan the service sleeps after inactivity, so the first
> request after idle takes ~50s to cold-start (and triggers one-time seeding of
> default content + the super admin).
